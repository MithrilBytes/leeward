...
| Same endpoint, next call | `DOWN{BREAKER_OPEN}` carrying `WEDGED`, 504, `DO_NOT_RETRY` | 0.7 ms | 0 | 0 |
| 429 with `Retry-After: 3600` | `DOWN{QUOTA_EXHAUSTED}`, 503, `DO_NOT_RETRY` | 3.3 ms | 1 | 1 |
| Static page, origin down | `STALE`, 200, `PROCEED_WITH_CAUTION` | 0.8 ms | 0 | 0 |
| Live endpoint, origin down | `DOWN{CONNECT_REFUSED}`, 503, `TREAT_AS_UNKNOWN` | 109 ms | 2 | 0 |
...
| Another tool on that server, next call | `FRESH`, `PROCEED` | 439 ms | 1 | 1 |
...
```text
[leeward] DOWN: 127.0.0.1 returned nothing (CONNECT_REFUSED after 107ms, 2 attempts).
