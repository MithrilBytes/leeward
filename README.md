...
- **A resource read** is cached without being asked, since the protocol says a resource read has no side effects. When the server stops answering, the document comes back exactly as it was sent, with the outcome in `_meta` rather than spliced into the text.
