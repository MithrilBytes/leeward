...
**MCP servers.** leeward fronts each configured server at its own path, with the same tool names.
...
**Everything else over HTTPS** is planned through a forward proxy on its own port, set with `HTTPS_PROXY`. A tunnel cannot see inside TLS, so that mode classifies failures and never caches. Not built yet.

**Model calls** are planned as an OpenAI compatible base URL, with tier failover and transport failures shaped like the API's own errors. Not built yet.
